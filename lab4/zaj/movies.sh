...
while getopts ":hd:t:a:fx" OPT; do
    case ${OPT} in
        h)
            print_help
            exit 0
...
if [[ -z ${MOVIES_DIR:-} ]]; then
    print_error "ERROR: dir cannot be empty"
    exit 1
fi
...
if ${SAVE_TO_FILE:-false}; then
    print_movies "${MOVIES_LIST}" "${OUTPUT_FORMAT:-raw}" | tee "${FILE_4_SAVING_RESULTS:-results.txt}"
else
